...
import { fetchDictionaries, fetchFishery, fetchFisheryRelations, saveFishery, adminSaveFishery, uploadFisheryPhoto, saveBathyImage, setFisheryRules, type FisheryRules } from '../lib/api';
...
const secId = (t: string) => 'sec-' + t.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
const SECTIONS: { full: string; short: string; icon: IconName; adminOnly?: boolean }[] = [
  { full: 'Plan łowiska', short: 'Plan', icon: 'card', adminOnly: true },
  { full: 'Podstawowe dane', short: 'Dane', icon: 'fish' },
  { full: 'Cennik', short: 'Cennik', icon: 'tag' },
  { full: 'Lokalizacja na mapie', short: 'Lokalizacja', icon: 'pin' },
  { full: 'Sezon i zasady rezerwacji', short: 'Sezon', icon: 'calendar' },
  { full: 'Typ łowiska', short: 'Typ', icon: 'tag' },
  { full: 'Kontakt', short: 'Kontakt', icon: 'phone' },
  { full: 'Gatunki ryb', short: 'Ryby', icon: 'fish' },
  { full: 'Udogodnienia', short: 'Udogodnienia', icon: 'layers' },
  { full: 'Rekordy łowiska', short: 'Rekordy', icon: 'trophy' },
  { full: 'Zdjęcia', short: 'Zdjęcia', icon: 'image' },
  { full: 'Mapa stanowisk', short: 'Stanowiska', icon: 'pin' },
  { full: 'Mapa batymetryczna', short: 'Batymetria', icon: 'droplet' },
  { full: 'Opis i regulamin', short: 'Opis', icon: 'edit' },
];

export default function FisheryForm({ admin = false }: { admin?: boolean }) {
  const { id } = useParams();
  const backTo = admin ? '/lowiska' : '/lowiska';
      ...
      try {
        ...
        if (id) {
          ...
          setV({
            ...
            price_tiers: f.price_tiers ?? [], total_spots: f.total_spots,
  ...
  useEffect(() => {
    if (loading) return;
    const els = SECTIONS.filter((s) => !s.adminOnly || admin).map((s) => document.getElementById(secId(s.full))).filter((e): e is HTMLElement => !!e);
  ...
  return (
    <>
      ...
      <div className="content">
        <SectionNav active={activeSec} admin={admin} onGo={goTo} />
        ...
        <form onSubmit={submit} style={{ maxWidth: 880, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 18 }}>
          ...
          <Card title="Cennik" icon="tag">
            <div className="hint" style={{ marginBottom: 12 }}>
              Trzy główne ceny — wpisz te, które oferujesz (puste pomijamy). „Cena od" w aplikacji policzy się z najniższej.
            </div>
            <div className="row">
          ...
          <Card title="Sezon i zasady rezerwacji" icon="calendar">
            ...
            <div className="row" style={{ gap: 14, flexWrap: 'wrap', marginTop: 12 }}>
              ...
              <Field label="Min. dni pobytu" style={{ flex: 1, minWidth: 120 }}><input className="input" type="number" min={1} value={rules.min_nights} onChange={(e) => setRule('min_nights', Number(e.target.value))} /></Field>
              <Field label="Maks. dni (puste = bez limitu)" style={{ flex: 1, minWidth: 140 }}><input className="input" type="number" min={1} value={rules.max_nights ?? ''} onChange={(e) => setRule('max_nights', e.target.value ? Number(e.target.value) : null)} /></Field>
          ...
          <Card title="Mapa stanowisk" icon="pin">
            {editing && v.id
              ? <SpotMapEditor fisheryId={v.id} layoutImageUrl={layoutUrl} />
              : <div className="hint">Zapisz najpierw łowisko (z liczbą stanowisk), a potem wróć tutaj, aby rozstawić stanowiska na planie.</div>}
          </Card>
          ...
          <div className="card" id={secId('Opis i regulamin')} style={{ scrollMarginTop: 74 }}>
...
function SectionNav({ active, admin, onGo }: { active: string; admin?: boolean; onGo: (id: string) => void }) {
  const secs = SECTIONS.filter((s) => !s.adminOnly || admin);
  return (
    <div className="form-nav">
      {secs.map((s) => {
        const id = secId(s.full);
        return (
          <button type="button" key={id} className={`form-nav-pill ${active === id ? 'on' : ''}`} onClick={() => onGo(id)}>
            <Icon name={s.icon} size={14} /> {s.short}
          </button>
        );
      })}
    </div>
...
function Card({ title, icon, children }: { title: string; icon: IconName; children: ReactNode }) {
  return (
    <div className="card" id={secId(title)} style={{ scrollMarginTop: 74 }}>
...
function PriceTiers({ value, onChange }: { value: PriceTier[]; onChange: (v: PriceTier[]) => void }) {
  const upd = (i: number, patch: Partial<PriceTier>) => onChange(value.map((t, idx) => (idx === i ? { ...t, ...patch } : t)));
  return (
    <div>
      {value.map((t, i) => (
        <div key={i} className="row" style={{ marginBottom: 8 }}>
          <input className="input" style={{ flex: 2 }} placeholder="Nazwa (np. Doba, Weekend, VIP)" value={t.label} onChange={(e) => upd(i, { label: e.target.value })} />
      ...
      <button type="button" className="btn ghost sm" onClick={() => onChange([...value, { label: '', price: 0 }])}><Icon name="plus" size={15} /> Dodaj wariant ceny</button>
    </div>
  );
}
