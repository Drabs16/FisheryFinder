...
import type { Fishery, Reservation, FisheryFormValues, AdminFisheryRow, AdminStats, ClaimRequestRow, Subscription } from './types';
...
export interface SpotRow {
  spot_number: number;
  pos_x: number | null; // 0..1 względem planu
  pos_y: number | null;
  has_power: boolean;
  is_vip: boolean;
  note: string | null;
...
export async function fetchSpots(fisheryId: string): Promise<SpotRow[]> {
  const { data, error } = await supabase
    .from('fishery_spots')
    .select('spot_number, pos_x, pos_y, has_power, is_vip, note')
...
export async function saveSpotLayout(fisheryId: string, spots: SpotRow[]): Promise<void> {
  const results = await Promise.all(spots.map((s) =>
    supabase.from('fishery_spots')
      .update({ pos_x: s.pos_x, pos_y: s.pos_y, has_power: s.has_power, is_vip: s.is_vip, note: s.note })
      .eq('fishery_id', fisheryId).eq('spot_number', s.spot_number)));
  const err = results.find((r) => r.error)?.error;
  if (err) throw err;
}

// Wgrywa plan/zdjęcie łowiska i zapisuje URL w fisheries.spot_map_url.
...
export interface OwnerReviewRow { id: number; fishery_id: string; fishery_name: string; author_name: string | null; rating: number; comment: string | null; visited_on: string | null; created_at: string | null; hidden: boolean; }
...
// ===========================================================================
// ADMIN OPS — POI / rezerwacje / połowy / jakość / ustawienia / admini / ogłoszenia
// ===========================================================================

export interface PoiRow {
  id: string; kind: 'shop' | 'pzw'; name: string; lat: number; lon: number;
  address: string | null; city: string | null; phone: string | null; website: string | null;
  verified: boolean; source: string | null; created_at: string | null;
}
export async function adminListPois(kind?: string, search?: string): Promise<PoiRow[]> {
  const { data, error } = await supabase.rpc('admin_list_pois', { p_kind: kind ?? null, p_search: search ?? null });
  if (error) throw error;
  return (data ?? []) as PoiRow[];
}
export async function adminSavePoi(p: Partial<PoiRow> & { kind: string; name: string; lat: number; lon: number }): Promise<string> {
  const { data, error } = await supabase.rpc('admin_save_poi', {
    p_id: p.id ?? null, p_kind: p.kind, p_name: p.name, p_lat: p.lat, p_lon: p.lon,
    p_address: p.address ?? null, p_city: p.city ?? null, p_phone: p.phone ?? null, p_website: p.website ?? null, p_verified: p.verified ?? false,
  });
  if (error) throw error;
  return data as string;
}
export async function adminDeletePoi(id: string): Promise<void> {
  const { error } = await supabase.rpc('admin_delete_poi', { p_id: id });
  if (error) throw error;
}
export async function adminSetPoiVerified(id: string, verified: boolean): Promise<void> {
  const { error } = await supabase.rpc('admin_set_poi_verified', { p_id: id, p_verified: verified });
  if (error) throw error;
}

export interface AdminReservationRow {
  id: string; fishery_id: string; fishery_name: string; name: string | null; phone: string | null; email: string | null;
  spots: number[] | null; date_from: string; date_to: string; days: number; total: number;
  payment: string | null; status: string; confirmed_at: string | null; created_at: string;
}
export async function adminListReservations(status?: string, search?: string, fishery?: string): Promise<AdminReservationRow[]> {
  const { data, error } = await supabase.rpc('admin_list_reservations', { p_status: status ?? null, p_search: search ?? null, p_fishery: fishery ?? null });
  if (error) throw error;
  return (data ?? []) as AdminReservationRow[];
}
export async function adminCancelReservation(id: string): Promise<void> {
  const { error } = await supabase.rpc('admin_cancel_reservation', { p_id: id });
  if (error) throw error;
}

export interface AdminCatchRow {
  id: string; species: string; weight: number | null; length_cm: number | null; spot_number: number | null;
  photo_url: string | null; caught_on: string; note: string | null; hidden: boolean; created_at: string;
  fishery_id: string; fishery_name: string | null; angler: string | null;
}
export async function adminListCatches(hidden?: string, search?: string): Promise<AdminCatchRow[]> {
  const { data, error } = await supabase.rpc('admin_list_catches', { p_hidden: hidden ?? null, p_search: search ?? null });
  if (error) throw error;
  return (data ?? []) as AdminCatchRow[];
}
export async function adminSetCatchHidden(id: string, hidden: boolean): Promise<void> {
  const { error } = await supabase.rpc('admin_set_catch_hidden', { p_id: id, p_hidden: hidden });
  if (error) throw error;
}

export interface DqNeed { id: string; name: string; city: string; province: string; premium: boolean; phone: string | null; has_desc: boolean; has_price: boolean; has_fish: boolean; has_photos: boolean; }
export interface DqDup { name: string; city: string; count: number; ids: string[]; }
export async function adminDataQuality(): Promise<{ needs: DqNeed[]; duplicates: DqDup[] }> {
  const { data, error } = await supabase.rpc('admin_data_quality');
  if (error) throw error;
  return (data ?? { needs: [], duplicates: [] }) as { needs: DqNeed[]; duplicates: DqDup[] };
}
...
export interface AdminAdmin { email: string; last_sign_in: string | null; is_self: boolean; }
export async function adminListAdmins(): Promise<AdminAdmin[]> {
  const { data, error } = await supabase.rpc('admin_list_admins');
  if (error) throw error;
  return (data ?? []) as AdminAdmin[];
}
export async function adminAddAdmin(email: string): Promise<void> {
  const { error } = await supabase.rpc('admin_add_admin', { p_email: email });
  if (error) throw error;
}
export async function adminRemoveAdmin(email: string): Promise<void> {
  const { error } = await supabase.rpc('admin_remove_admin', { p_email: email });
  if (error) throw error;
}

export interface AnnouncementRow { id: string; title: string; body: string; audience: 'all' | 'anglers' | 'owners'; active: boolean; created_at: string; }
export async function adminListAnnouncements(): Promise<AnnouncementRow[]> {
  const { data, error } = await supabase.rpc('admin_list_announcements');
  if (error) throw error;
  return (data ?? []) as AnnouncementRow[];
}
export async function adminSaveAnnouncement(a: { id?: string | null; title: string; body: string; audience: string; active: boolean }): Promise<void> {
  const { error } = await supabase.rpc('admin_save_announcement', { p_id: a.id ?? null, p_title: a.title, p_body: a.body, p_audience: a.audience, p_active: a.active });
  if (error) throw error;
}
...
// Szybka edycja łowiska (podstawowe pola) — bez pełnego formularza.
export async function adminQuickUpdateFishery(p: { id: string; name: string; city: string; province: string; price_from: number | null; price_24h: number | null; price_day: number | null; price_night: number | null }): Promise<void> {
  const { error } = await supabase.rpc('admin_quick_update_fishery', {
    p_id: p.id, p_name: p.name, p_city: p.city, p_province: p.province,
    p_price_from: p.price_from, p_price_24h: p.price_24h, p_price_day: p.price_day, p_price_night: p.price_night,
  });
  if (error) throw error;
}
