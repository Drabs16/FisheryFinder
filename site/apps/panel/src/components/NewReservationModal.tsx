...
export default function NewReservationModal({ fisheries, reservations = [], defaultFisheryId, defaultSpot, onClose, onCreated }: Props) {
  ...
  // Doby = noce (przyjazd→wyjazd), spójnie z owner_create_reservation. Jeden dzień (od=do) = 1 doba.
    ...
    <div className="modal-back" onClick={onClose}>
      <div className="modal" style={{ maxWidth: 560 }} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="row">
          ...
          <div className="field" style={{ flex: 1 }}><label>Cena / doba (zł)</label><input className="input" type="number" min={0} value={price} onChange={(e) => setPrice(Number(e.target.value))} /></div>
        </div>
